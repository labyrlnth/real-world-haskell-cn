.. _chapter-10-code-case-study-parsing-a-binary-data-format:
...
.. note::
    ...
    我们之前还没有讨论过 ``parseByte`` 函数的视觉风格。它将匿名函数的参数和 ``->`` 放在行尾，函数体放在下一行。

    这种布局的匿名函数没有正式名字，因此让我们称它为“悬挂 lambda”，它的主要作用是给函数体留出更多空间。
    它也让我们更清楚地看到相邻两个函数之间的关系。例如，第一个函数的结果经常作为参数传给第二个函数。
